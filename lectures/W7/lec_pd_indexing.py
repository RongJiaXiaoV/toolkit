...
#   Outline:检索
#
#   1. Selection using loc (label based)按照行和列的标签进行检索
#     1.1 Series:
...
#
#   2. Selection using iloc (position based) 按行和列的位置检索
#     2.1 Series:
...
# will return a series 同时检索两个日期的数据
...
x  = df.loc[:,'Close']
#x  = df.loc['Close'] 是错误的，逗号前不能省略，检索列
#print(x)
...
# 1.2.2 Dataframe.loc: Selection using sequence of labels 可以有同时检索多个标签
# Set x so it contains the closing prices for '2020-01-02' and '2020-01-03'
#x  = df.loc[['2020-01-02', '2020-01-03'], 'Close']
#print(x)
#print(type(x))

# 1.2.3 Dataframe.loc: Selection using slices 切片，检索1月6号后的所有数据
...
# NOTE: don't worry about the rename method now 切片前要排序？
...
# print(x)
# 用标签切片有两行，注意
# y = ser.loc['2020_01_02':'2020_01_03']
# print(y)
x  = ser.iloc[0:2]
#print(x)
